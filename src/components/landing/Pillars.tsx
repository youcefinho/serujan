...
const icons = [ShieldCheck, Crosshair, BookOpen, MapPin];

export function Pillars() {
  const { t, ta } = useLanguage();
  const pillars = ta(translations.pillars.items) as { title: string; text: string }[];
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="mb-16 md:text-center">
          <h2 className="text-3xl md:text-5xl font-bold mb-6 text-balance uppercase tracking-widest">
            {t(translations.pillars.title)}
          </h2>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            {t(translations.pillars.subtitle)}
          </p>
        </div>
